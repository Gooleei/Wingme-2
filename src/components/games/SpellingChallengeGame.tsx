...
export const SpellingChallengeGame: React.FC<SpellingChallengeProps> = ({
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-6 space-y-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-black text-white flex items-center gap-2">
              <span>🔤 Spelling Challenge (3s Rush)</span>
              <span className="text-xs px-2 py-0.5 rounded-full bg-emerald-500/20 text-emerald-400 border border-emerald-500/30">
                +${rewardAmount.toFixed(2)} Win
              </span>
            </h1>
            <p className="text-xs text-slate-400">Type the highlighted word in under 3 seconds</p>
          </div>
        </div>
      ...
      <div className="bg-slate-950 border-2 border-slate-800 rounded-3xl p-6 text-center space-y-5 shadow-2xl relative overflow-hidden">
        <div className="flex justify-between items-center text-xs text-slate-400 font-bold px-2">
          ...
          <span className={`flex items-center gap-1 font-mono text-sm ${timeLeftMs < 1000 ? 'text-rose-500 animate-pulse' : 'text-cyan-400'}`}>
        ...
        {/* Big Word Card */}
        <div className="py-6 px-4 bg-slate-900/60 rounded-2xl border border-slate-800/80">
          <span className="text-xs uppercase font-bold tracking-widest text-slate-500">Target Word</span>
          <div className="text-4xl sm:text-5xl font-black text-transparent bg-clip-text bg-gradient-to-r from-amber-300 via-emerald-300 to-cyan-300 tracking-wider mt-2">
            {targetWord}
          </div>
        </div>
        ...
        <div className="relative max-w-sm mx-auto">
          <input
            ...
            disabled={!isPlaying}
            placeholder={isPlaying ? 'TYPE HERE...' : 'Press Start Round'}
            className="w-full px-4 py-3.5 rounded-xl bg-slate-900 border-2 border-cyan-500/50 text-white font-mono font-black text-2xl text-center uppercase tracking-widest focus:outline-none focus:border-cyan-400 placeholder:text-slate-600 placeholder:text-sm placeholder:font-sans shadow-inner"
          />
        </div>
          ...
          <div className="flex flex-wrap justify-center gap-1 max-w-md mx-auto">
            {letters.map((l) => (
              <button
                ...
                disabled={!isPlaying}
                className="w-8 h-9 rounded-lg bg-slate-800 hover:bg-slate-700 active:bg-cyan-500 active:text-slate-950 text-white text-xs font-bold transition-colors border border-slate-700 cursor-pointer disabled:opacity-40"
              >
                {l}
              </button>
            ))}
            <button
              ...
              disabled={!isPlaying}
              className="px-3 h-9 rounded-lg bg-rose-950/60 hover:bg-rose-900/60 text-rose-300 text-xs font-bold border border-rose-800/60 cursor-pointer disabled:opacity-40"
            >
              ⌫ Del
            </button>
          </div>
            ...
            <div className="bg-slate-900 border-2 border-slate-700 rounded-3xl p-6 text-center space-y-4 max-w-sm w-full shadow-2xl">
              {gameResult === 'WIN' ? (
                <>
                  ...
                  <div>
                    <h3 className="text-xl font-black text-white">Lightning Fast!</h3>
                ...
                <>
                  ...
                  <div>
                    <h3 className="text-xl font-black text-white">Time Expired!</h3>
                    <p className="text-xs text-slate-400 mt-1">Keep training your keyboard agility.</p>
                  </div>
                </>
              )}
